...
curl -v -F "status=2" -F "notifytatus=2" -F "notify=1" -F "ipa=@${path}" $(curl -X POST "${appcenterUrl}" -H "accept: application/json" -H "X-API-Token: ${appcenterToken}" -H "Content-Type:application/json" -d "{ \"release_id\": 0}" | jq -r '.upload_url') &&
